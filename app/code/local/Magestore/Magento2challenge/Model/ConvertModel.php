...
class Magestore_Magento2challenge_Model_ConvertModel extends Mage_Core_Model_Abstract
{
    ...
    public function convert($file){
        ...
        //Mage::helper('magento2challenge')->writeContentToFile('log.txt', $file);
        ...
        $isFile = 'model';
        $resourceFolder = 'Mysql4'; //default resource folder
        $resourceClass = $this->_helper->getResourceClass();
        ...
        if($isFile == 'model'){
            ...
            $pathFile = str_replace('\\', DS, $this->_newNamespace .'\\'.$this->_newClassName);
            $pathFile = 'app'.DS.'code'.DS.$pathFile.'.php';
            Mage::helper('magento2challenge')->writeContentToFile($pathFile, $this->_newContent);
        }elseif($isFile == 'resource'){
            //starting convert resource
            Mage::getModel('magento2challenge/convertResource')
                ->convert($file, $resourceFolder);
        }elseif($isFile == 'collection'){
            //starting convert resource collection
            Mage::getModel('magento2challenge/convertCollection')
                ->convert($file, $resourceFolder);
        }else{
    ...
    private function _initFile($file){
        ...
        $this->_construct = isset($this->_allFunctions['__construct'])? $this->_allFunctions['__construct'] : '';
        //$this->_helper->getFunctionContentAll($this->_content, '__construct');
        return $this;
    }
    ...
    private function _convertConstruct(){
        ...
        if(isset($this->_allFunctions['_construct'])){
            $_extraContent = $this->_helper->getFunctionContent('<?php '.$this->_allFunctions['_construct'], '_construct');
            //remove old parent::_construct
            $_extraContent = preg_replace('/parent\s*::\s*_construct\s*.*?\)\s*;/', '', $_extraContent);
            //remove _init function call
            $_init = $this->_helper->searchLine('_init', '<?php '.$_extraContent);
            $_extraContent = str_replace($_init, '', $_extraContent);
        }
        $_construct_content .= PHP_EOL.$_extraContent;
    ...
    private function _getAllFunctions(){
        $this->_allFunctions = $this->_helper->getAllFunctions($this->_content);
