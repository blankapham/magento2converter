 ...
 */
class Magestore_Magento2challenge_Model_ConvertResource extends Mage_Core_Model_Abstract
{
    protected $_helper;
    ...
    public function _construct()
    {
        ...
        parent::_construct();
        $this->_init('magento2challenge/convertResource');
    }
    ...
    private function _initFile($file){
        ...
        $this->_maintable = $this->_helper->getMainTable($this->_getMainTablePath());
    ...
    private function _convertConstruct(){
        ...
        if(isset($this->_allFunctions['_construct'])){
            //convert init
            $this->_allFunctions['_construct'] = preg_replace(
                '/\$this\s*->\s*_init\s*\(\s*\\\'(.*?)\\\'\s*,\s*\\\'(.*)\\\'\s*\)/',
                '$this->_init(\''.$this->_maintable.'\', \'$2\')',
                $this->_allFunctions['_construct']);
        }
    ...
    private function _getMainTablePath(){
        if(preg_match('/\$this\s*->\s*_init\s*\(\s*["\'](.*)["\']\s*,\s*.*\s*\)/', $this->_content, $matched)){
            return $matched[1];
        }
        return '';
    }
